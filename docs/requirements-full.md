...
>
> **SSRF guard (rewritten 2026-08-13).** Every fetch of a user-supplied URL now goes through `api/utils/safe_fetch.safe_request` / `safe_stream`, which per redirect hop: rejects non-`http(s)` schemes, resolves the hostname and **rejects the host if any returned address is blocked**, then connects **pinned to the validated address** while preserving the `Host` header and the TLS SNI/certificate hostname. Pinning closes DNS rebinding (the resolved address is the one connected to, so there is no time-of-check/time-of-use window), and per-hop revalidation closes redirect-based bypass. Blocked address classes come from Python's `ipaddress` (`is_private`, `is_loopback`, `is_link_local`, `is_reserved`, `is_multicast`, `is_unspecified`, with `ipv4_mapped` unwrapped) plus CGNAT `100.64.0.0/10`, `192.0.0.0/24` and `198.18.0.0/15`; resolution also normalises decimal/octal/short-dotted literal forms for free. The previous hostname-resolution gap and three previously unguarded fetchers (`feed_discovery.discover_feed`, `feeds._seed_feed_history`, `worker.feed_poll`) are closed. `POST /api/feeds/discover` now requires auth and is rate limited (`FEEDS_DISCOVER_PER_MINUTE`). Self-hosters on a private LAN can opt back in with `ALLOW_PRIVATE_NETWORK_FETCH=true` — a behaviour change for anyone who saved LAN links before. Covered by `tests/test_ssrf_guard.py` (policy + pinning + call sites, incl. a source scan that fails if a new fetcher bypasses the module) and `tests/integration/test_outbound_guard.py` (the DB-writing paths).
...
| Ollama Cloud requests keep the credential on-origin | `tests/test_ollama_cloud.py` (bearer key, constant host, `/api/ps` probe fails closed on 401/403 and open on 404/405), `tests/test_ssrf_guard.py` (pinned to the validated address with `Host: ollama.com` preserved; `Authorization` dropped on a cross-origin redirect hop) |
