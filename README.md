...
Five steps, each with a checkpoint so you know it worked before moving on.
Total: two commands and a couple of clicks, plus one slow first build.

### Prerequisites

- Docker and Docker Compose v2 (`docker compose`, not `docker-compose`)
- ~4 GB free disk and 2 GB RAM for the stack
- Optional: `make`, which wraps every command below (`make help` lists them)
...
cd Arciv
./scripts/bootstrap-env.sh          # or: make setup
```

This copies `.env.example` to `.env` and fills in a real `SECRET_KEY` and
`ENCRYPTION_KEY`. Every other default already targets the local Compose stack,
so nothing else has to be edited to get running. It also warns you if any of the
published ports are already taken.

> **Checkpoint:** `.env` exists, and `grep SECRET_KEY .env` shows 64 hex
> characters rather than `change_me_...`.

### 2. (Optional) Add an AI key

Arciv works without one — links route by URL heuristics and everything else
behaves normally. To get summaries and tags, either:

- Set `SHARED_GEMINI_KEY` in `.env` (server-wide, capped at 20 calls/user/day), or
- Skip this and paste a personal key into **Settings** after logging in.

Free keys: [Google AI Studio](https://aistudio.google.com/apikey) (Gemini) or
...
**The first run takes roughly 5–10 minutes** and looks idle for long stretches.
That is expected — it is building, not hung:


| Stage                       | Why                                                                                                                         |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `npm ci` + Vite build       | the React SPA is baked into the API image                                                                                   |
| `pip install`               | the Python runtime for the api and worker                                                                                  |
| ~400 MB model download (×2) | the ONNX embedding model, baked into the api image *and* the embed service, so containers start with no cold-start download |


That model is what makes search match by meaning instead of substrings. Later
runs reuse the Docker cache and start in seconds.
...
1. Open [**http://localhost:8000**](http://localhost:8000) and register.
2. **Login requires a verified email.** Open [**http://localhost:8025**](http://localhost:8025) — the
 bundled MailHog inbox — and click the verification link in the message that
 just arrived.
3. Log in.

The default `.env` points SMTP at MailHog exactly so this loop works with no
mail provider. If you set `EMAIL_ENABLED=false`, nothing is sent and the link is
only written to the worker log:
`docker compose logs worker | grep verify-email`.

> **Checkpoint:** you are logged in and looking at an empty Inbox queue.

### 5. Try the actual features

1. **Save a link** — paste a URL into the input bar. It appears immediately;
 title and metadata fill in within a second.
2. **Watch it route** — with an AI key it gets a summary, tags and a queue
 within a few seconds (`docker compose logs -f worker` shows the job). Without
 one it routes by URL heuristics: a YouTube link lands in Watch Later, a
 GitHub link in Try Later.
3. **Search it** — the search box matches by meaning, not just substring. Save
 an article about databases, then search `storage engine`.
4. **Filter by topic** — the Topics panel lists tags scoped to the current
 queue.
5. **Subscribe to a feed** — Feeds → paste a blog URL; Arciv discovers the RSS
 feed. Feeds notify you about new posts, they don't auto-save them.
...
| ------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `403 Email not verified` at login     | Step 4 — click the link in [http://localhost:8025](http://localhost:8025)                                |
| `bind: address already in use`        | Something holds 8000, 5432, 6379, 8001, 1025 or 8025. Stop it, or remap `ports:` in `docker-compose.yml` |
| Build seems stuck with no output      | The model download. Give it a few minutes; `docker compose logs -f` shows progress                       |
| Search returns 503                    | `EMBEDDINGS_ENABLED=false`, or the `embed` service is down (`docker compose ps`)                         |
| Links stay in Inbox, never summarised | No AI key configured. Expected — set one in Settings, or ignore it                                       |
| Frontend edits don't show at :8000    | The SPA is baked into the image. Use the Vite dev server (see [Development](#development))               |
...
├── worker/             # ARQ jobs — ai_classify, feed_poll, embeddings, email
├── db/migrations/      # Alembic versions
├── frontend/           # React SPA (Vite + Tailwind)
├── docs/               # roadmap, MVP + full spec, link state diagram
├── docker-compose.yml
...
[Makefile](Makefile) wraps the commands in this section — run `make` for the
list. Everything it does is a plain `docker compose` / `pytest` invocation, so
...
```bash
make venv        # python3.12 venv + runtime and dev dependencies
make test-unit   # unit layer only — no services needed, ~3s
make test        # full suite: starts throwaway Postgres + Redis, then pytest
make lint        # ruff
```

`make test` brings up [docker-compose.test.yml](docker-compose.test.yml) — a
disposable pgvector + Redis pair on ports **55432/56379**. The non-default ports
are deliberate: the suite `TRUNCATE`s every table between tests, so it must not
be able to reach your dev database on 5432. `make test-services-down` removes
them.
...
```bash
cp .env.example .env
# Edit .env: set ENVIRONMENT=production, generate fresh secrets
...
```env
POSTGRES_PASSWORD=[[ORCA_RICH_MD:c29a8f1abcd1fe4d197a611d5cbb76ff:inline-html:%3Cgenerate%20one%3E]]
DATABASE_URL=postgresql://arciv:[[ORCA_RICH_MD:c29a8f1abcd1fe4d197a611d5cbb76ff:inline-html:%3Csame%20password%3E]]@db:5432/arciv
```
...
Looking for something to pick up? The [roadmap](docs/roadmap.md) lists what's planned and what's already done.
